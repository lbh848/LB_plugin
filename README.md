...
- `module/라이트보드  삽화 3.4.1-soya-v47.module.charx`: 현재 플러그인과 함께 사용하는 모듈
...
호환 구성은 soya-v47 모듈과 illustration context bridge version 7 서버입니다. 키비주얼은 서버 이미지 URL로 GIF 애니메이션을 표시하면서 같은 결과의 Risu 로컬 인레이를 fallback으로 함께 보존합니다. 서버 세션이나 임시 HTTPS 주소가 사라진 오래된 채팅은 GIF 대신 로컬 첫 프레임을 표시하므로 검은 박스로 남지 않습니다. 일반 장면 이미지는 기존 Risu 인레이 경로로 처리합니다. 플러그인이 꺼져 있어도 이미지 생성·편하게 수정 기능은 독립적으로 작동합니다. 일반 생성과 전체·그림별 재생성 및 편하게 수정은 시작 전에 서버 연결을 최대 5초만 확인하고, 연결되지 않으면 생성 요청 없이 즉시 오류로 종료합니다.
