...
- `module/라이트보드  삽화 3.4.1-soya-v46.module.charx`: 현재 플러그인과 함께 사용하는 모듈
...
4. generation, CALL1부터 시작하는 전체 재생성, raw 전체 생성, 이미지별 raw 재생성·편하게 수정 때 진행창이 표시됩니다.
5. 이미지 호버의 `✎` 버튼을 누르면 수정 방향 입력창이 열리고, 백엔드의 기존 편하게 수정 LLM과 수정 재생성을 거쳐 해당 이미지만 교체됩니다.
6. 진행창 표시를 꺼도 생성 연동과 필요한 동안의 상태 확인은 계속 작동합니다.
...
호환 구성은 soya-v46 모듈과 illustration context bridge version 7 서버입니다. 키비주얼은 애니메이션 보존을 위해 서버 이미지 URL을 직접 표시하고, 일반 장면 이미지는 기존 Risu 인레이 경로로 처리합니다. 플러그인이 꺼져 있어도 이미지 생성·편하게 수정 기능은 독립적으로 작동합니다. 일반 생성과 전체·그림별 재생성 및 편하게 수정은 시작 전에 서버 연결을 최대 5초만 확인하고, 연결되지 않으면 생성 요청 없이 즉시 오류로 종료합니다.
