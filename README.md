...
- `module/라이트보드  삽화 3.4.1-soya-v45.module.charx`: 현재 플러그인과 함께 사용하는 모듈
...
호환 구성은 soya-v45 모듈과 illustration context bridge version 7 서버입니다. 모듈은 이미지와 실제 슬롯을 Risu 인레이 경로로 직접 처리하며, 플러그인이 꺼져 있어도 이미지 생성·편하게 수정 기능은 독립적으로 작동합니다. 일반 생성과 전체·그림별 재생성 및 편하게 수정은 시작 전에 서버 연결을 최대 5초만 확인하고, 연결되지 않으면 생성 요청 없이 즉시 오류로 종료합니다.
