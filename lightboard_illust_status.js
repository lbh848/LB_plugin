...
//@display-name soya comfy manager plugin v1.0.15
//@api 3.0
//@version 42.0.24
//@author soya
...
// Configuration/status-only companion for soya-v51.
...
(async () => {
  ...
  const renderDashboard = (force = false) => {
    ...
    document.body.innerHTML = `
      ...
      <main id="${ROOT_ID}"><div class="shell">
        <header><div><h1>soya comfy manager v1.0.15</h1><div class="sub">v42.0.24 · 자동/수동 삽화 버튼 감시 · 에셋 표시 크기 분리</div></div><button id="lb-v42-close">닫기</button></header>
  ...
  const getCompatibleHealth = async () => {
    ...
    if (health?.ok !== true || safeNumber(health?.version) < 9
      ...
      throw new Error('server does not advertise the v42 protocol 9 controls');
    }
    healthCheckedAt = now;
    return health;
  };
