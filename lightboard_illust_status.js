...
//@display-name soya comfy manager plugin v1.0.14
//@api 3.0
//@version 42.0.23
//@author soya
...
// Configuration/status-only companion for soya-v50.
...
(async () => {
  ...
  const renderDashboard = (force = false) => {
    ...
    document.body.innerHTML = `
      ...
      <main id="${ROOT_ID}"><div class="shell">
        <header><div><h1>soya comfy manager v1.0.14</h1><div class="sub">v42.0.23 · 자동/수동 삽화 버튼 감시 · 애니메이션 깜빡임 완화</div></div><button id="lb-v42-close">닫기</button></header>
  ...
  try {
    ...
    console.log(LOG, 'plugin.boot', JSON.stringify({
      ...
      module_image_transport: 'all-slots-animated-eager-sync-server-url+persistent-risu-png-fallback',
