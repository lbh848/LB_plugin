...
//@display-name soya comfy manager plugin v1.0.11
//@api 3.0
//@version 42.0.20
//@author soya
...
// Configuration/status-only companion for soya-v47.
...
(async () => {
  ...
  const renderDashboard = (force = false) => {
    ...
    document.body.innerHTML = `
      ...
      <main id="${ROOT_ID}"><div class="shell">
        <header><div><h1>soya comfy manager v1.0.11</h1><div class="sub">v42.0.20 · 자동/수동 삽화 버튼 감시 · GIF 키비주얼 로컬 fallback</div></div><button id="lb-v42-close">닫기</button></header>
  ...
  try {
    ...
    console.log(LOG, 'plugin.boot', JSON.stringify({
      ...
      key_visual_transport: 'animated-server-url+persistent-risu-fallback',
